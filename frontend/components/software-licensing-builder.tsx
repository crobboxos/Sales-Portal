...
export function SoftwareLicensingBuilder() {
  const [accountSearch, setAccountSearch] = useState("");
  const [accounts, setAccounts] = useState<
    Array<{ sfAccountId: string; name: string; accountNumber?: string }>
  >([]);
  ...
  const [serverErrors, setServerErrors] = useState<
    Array<{ portalField: string; message: string; severity: string }>
  >([]);
  ...
  const updateLine = (
    index: number,
    updater: (line: DealLineDraft) => DealLineDraft
  ): void => {
    setPayload((current) => {
  ...
  const onProductSelected = (index: number, pricebookEntryId: string): void => {
    const selected = products.find(
      (product) => product.sfPricebookEntryId === pricebookEntryId
    );
    if (!selected) {
      return;
    }
  ...
  const onSubmit = async (): Promise<void> => {
    ...
    if (localErrors.some((entry) => entry.severity === "block")) {
      setStatus({
        type: "error",
        message: "Fix blocking validation errors before submit."
      });
      return;
    }
    ...
    try {
      ...
      setStatus({
        ...
        message:
          details.message ??
          "Submission failed. Check Salesforce field errors and try again."
      });
    }
  };
        ...
        <div className="app-container">
          <section className="panel dashboard-head">
            <div>
              ...
              <p className="dashboard-copy">
                Creates Salesforce opportunities and line items using Composite
                API idempotent upsert with portal-owned delivery lines.
              </p>
            </div>
          ...
          <section className="dashboard-metrics">
            ...
            <article className="metric-card">
              ...
              <span className="metric-value">
                {payload.lines.length.toString().padStart(2, "0")}
              </span>
            </article>
            <article className="metric-card">
              ...
              <div className="panel-head">
                <div>
                  ...
                  <p className="panel-subtitle">
                    Search accounts and set header fields before building line
                    items.
                  </p>
                </div>
                    ...
                    >
                      {selectedFundingOptions.map((fundingType) => (
                        <button
                          ...
                          key={fundingType}
                          className={
                            payload.fundingType === fundingType ? "active" : ""
                          }
                          onClick={() =>
                            setPayload((current) => ({
                              ...current,
            ...
            <div className="panel-body">
              <div className="panel-head">
                <div>
                  ...
                  <p className="panel-subtitle">
                    Configure products, pricing, and subscription terms per
                    line.
                  </p>
                </div>
                    ...
                    <div className="form-grid cols-3">
                      <label className="field">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            onProductSelected(index, event.target.value)
                          }
                        >
                          <option value="">Select product</option>
                          {products.map((product) => (
                            <option
                              key={product.sfPricebookEntryId}
                              value={product.sfPricebookEntryId}
                            >
                              {product.name} ({product.currencyIsoCode}{" "}
                              {toNumberDisplay(Number(product.unitPrice))})
                            </option>
                          ))}
                        </select>
                      </label>
                      ...
                      <label className="field">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            updateLine(index, (currentLine) => ({
                              ...currentLine,
                              productCondition: event.target
                                .value as DealLineDraft["productCondition"]
                            }))
                          }
                        >
                    ...
                    {payload.fundingType === "Subscription" ? (
                      <div className="subsection">
                        <div className="panel-head">
                          ...
                          <span className="status-badge status-cyan">
                            Recurring
                          </span>
                        </div>
            ...
            <div className="panel-body">
              <div className="panel-head">
                <div>
                  ...
                  <p className="panel-subtitle">
                    Operational readout of all current line values.
                  </p>
                </div>
                <span className="status-badge status-cyan">
                  {payload.lines.length.toString().padStart(2, "0")} rows
                </span>
              </div>
                    ...
                    {payload.lines.length === 0 ? (
                      ...
                      payload.lines.map((line, index) => {
                        const selectedProduct = products.find(
                          (product) =>
                            product.sfPricebookEntryId === line.pricebookEntryId
                        );
                        return (
            ...
            <div className="panel-body">
              <div className="panel-head">
                <div>
                  ...
                  <p className="panel-subtitle">
                    Validate the draft, then submit to Salesforce.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="actions">
                <span className="status-badge status-neutral">
                  StageName=Open and LOB=Software Licensing
                </span>
              </div>
              ...
              {status.type === "success" ? (
                <div className="alert success">
                  ...
                  {status.sfOppId ? (
                    <div className="small-note data-text">
                      Salesforce Opportunity ID: {status.sfOppId}
                    </div>
                  ) : null}
                </div>
              ) : null}
              ...
              {status.type === "running" ? (
                <div className="alert info">
                  Composite API submission in progress...
                </div>
              ) : null}
            </div>
          </section>
