...
function isTransientCompositeFailure(response) {
  return response.compositeResponse.some((item) => item.httpStatusCode >= 500);
}
function buildCompositePayload(deal, lines) {
  const compositeRequest = [
    {
      method: "PATCH",
      url: `/services/data/v${env_1.env.SF_API_VERSION}/sobjects/Opportunity/Portal_Deal_Id__c/${deal.portal_deal_id}`,
      referenceId: "opp",
      body: {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    compositeRequest.push({
      method: "PATCH",
      url: `/services/data/v${env_1.env.SF_API_VERSION}/sobjects/OpportunityLineItem/Portal_Line_Id__c/${line.portal_line_id}`,
...
async function ensureDeliveryLine(deal, lines) {
  const hasDeliveryLine = lines.some((line) => line.is_delivery_line);
  if (hasDeliveryLine || deal.lines_of_business !== "Software Licensing") {
    return lines;
  }
  const deliveryEntry = await (0,
  referenceRepository_1.getDeliveryPricebookEntry)(
    env_1.env.SF_DELIVERY_PRODUCT_CODE_SOFTWARE_LICENSING,
    deal.currency_iso_code,
    deal.pricebook2_id
  );
  if (!deliveryEntry) {
    throw new Error(
      `Delivery pricebook entry not found for ${env_1.env.SF_DELIVERY_PRODUCT_CODE_SOFTWARE_LICENSING}/${deal.currency_iso_code}/${deal.pricebook2_id}`
    );
  }
  await (0, dealRepository_1.insertLineItem)(deal.id, {
    portalLineId: (0, uuid_1.v4)(),
  ...
  const refreshed = await (0, dealRepository_1.getDealById)(deal.id);
  if (!refreshed) {
    throw new Error("Deal disappeared while creating delivery line.");
  }
  return refreshed.lines;
}
async function readBackOpportunity(portalDealId) {
  const soql = `
    ...
    LIMIT 1
  `;
  return salesforceClient_1.salesforceClient.querySingle(soql);
}
async function readBackLines(portalLineIds) {
  ...
  `;
  return salesforceClient_1.salesforceClient.queryAll(soql);
}
async function handleTransientFailure(submissionId, errorDetail, retryCount) {
  const nextAttempt = retryCount + 1;
  if (nextAttempt > retryQueue_1.retryQueue.maxAttempts()) {
    await (0, submissionRepository_1.updateSubmissionStatus)(
      submissionId,
      "dead_letter",
      {
        errorDetail,
  ...
  await retryQueue_1.retryQueue.scheduleRetry(submissionId, nextAttempt);
...
async function submitDeal(
  ...
  const payload = await (0, dealRepository_1.getDealById)(dealId);
  if (!payload) {
    throw new Error("Deal not found.");
  }
  const deal = payload.deal;
  const lines = await ensureDeliveryLine(deal, payload.lines);
  const validationIssues = await (0, validationService_1.validateDealForSubmit)(
    deal,
    lines
  );
      ...
      errors: [
        {
          ...
          portalField: "form",
          message:
            "Salesforce credentials are missing. Configure SF_CLIENT_ID/SF_USERNAME/SF_PRIVATE_KEY.",
  ...
  const submission = existingSubmissionId
    ? await (0, submissionRepository_1.getSubmissionById)(existingSubmissionId)
    : await (0, submissionRepository_1.createSubmission)(deal.id, submittedBy);
  if (!submission) {
  ...
  await (0, dealRepository_1.updateDealStatus)(deal.id, "pending_submission", {
    markSubmitted: true
  });
  await (0, auditRepository_1.createAuditEvent)({
  ...
  try {
    const response =
      await salesforceClient_1.salesforceClient.composite(compositePayload);
    const errors = (0, errorParser_1.parseCompositeErrors)(response);
    if (errors.length > 0) {
      ...
      await (0, dealRepository_1.updateDealStatus)(deal.id, "failed");
      await (0, submissionRepository_1.createSyncJob)({
    ...
    if (!readBack) {
      throw new Error(
        "Salesforce composite succeeded but opportunity read-back returned no record."
      );
    }
    ...
    await (0, dealRepository_1.attachSalesforceLineIds)(lineIdMap);
    await (0, dealRepository_1.updateDealStatus)(deal.id, "synced", {
    ...
    return {
      ...
      submissionId: submission.id,
      sfOppId: readBack.Id,
      sfOppUrl: `${env_1.env.SF_LOGIN_URL}/lightning/r/Opportunity/${readBack.Id}/view`,
      status: "synced",
      warnings
    };
  } catch (error) {
    ...
    if (transient) {
      const status = await handleTransientFailure(
        submission.id,
        { message },
        submission.retry_count
      );
      await (0, dealRepository_1.updateDealStatus)(deal.id, "pending_retry");
      await (0, auditRepository_1.createAuditEvent)({
        entityType: "submission",
        entityId: submission.id,
...
async function retrySubmission(submissionId, attempt, userId = "system-retry") {
  const submission = await (0, submissionRepository_1.getSubmissionById)(
    submissionId
  );
  if (!submission) {
    throw new Error(`Submission ${submissionId} not found.`);
  }
  const dealPayload = await (0, dealRepository_1.getDealById)(
    submission.deal_id
  );
  if (!dealPayload) {
    throw new Error(
      `Deal ${submission.deal_id} not found for submission ${submissionId}.`
    );
  }
  await (0, auditRepository_1.createAuditEvent)({
    entityType: "submission",
