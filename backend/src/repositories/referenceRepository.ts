...
export interface PricebookEntryRefInput {
...
export async function upsertAccountRef(input: AccountRefInput, client?: PoolClient): Promise<void> {
...
export async function upsertProductRef(input: ProductRefInput, client?: PoolClient): Promise<void> {
  const runner = client ?? query;
  const sql = `
...
export async function upsertPricebookEntryRef(input: PricebookEntryRefInput, client?: PoolClient): Promise<void> {
  const runner = client ?? query;
  const sql = `
...
export async function upsertPicklistValues(
  objectApiName: string,
  fieldApiName: string,
...
export async function findAccounts(search?: string, limit = 25): Promise<unknown[]> {
  const sql = `
    SELECT
  ...
  const result = await query(sql, [search ?? null, limit]);
  return result.rows;
}

export async function findProducts(lob?: string, currency?: string, limit = 100): Promise<unknown[]> {
  const sql = `
    SELECT
  ...
  const result = await query(sql, [lob ?? null, currency ?? null, limit]);
  return result.rows;
}

export async function getPicklistValues(objectApiName: string, fieldApiName: string): Promise<unknown[]> {
  const sql = `
    SELECT
      value,
      label,
  ...
  const result = await query(sql, [objectApiName, fieldApiName]);
  return result.rows;
}

export async function getDeliveryPricebookEntry(
  productCode: string,
  currencyIsoCode: string,
  pricebook2Id: string
): Promise<
...
> {
  ...
  const result = await query<{
    ...
    sfProduct2Id: string;
    unitPrice: number;
  }>(sql, [productCode, currencyIsoCode, pricebook2Id]);
