...
import { DealLineRecord, DealRecord } from "../models/deal";
...
async function loadPicklists(fields: Array<{ objectApiName: string; fieldApiName: string }>): Promise<PicklistCache> {
  ...
  for (const field of fields) {
    ...
    cache[key] = new Set(result.rows.map((row: { value: string }) => row.value));
  }

  return cache;
}

function checkPicklist(cache: PicklistCache, objectApiName: string, fieldApiName: string, value: string | null): boolean {
  if (!value) {
    return false;
  }
...
async function lookupPricebookCurrency(pricebookEntryId: string): Promise<string | null> {
  const result = await query<{ currency_iso_code: string }>(
...
async function lookupProductCategory(productId: string): Promise<string | null> {
  const result = await query<{ product_category: string | null; line_of_business: string | null }>(
...
export async function validateDealForSubmit(deal: DealRecord, lines: DealLineRecord[]): Promise<ValidationIssue[]> {
  ...
  const picklistCache = await loadPicklists([
    ...
    { objectApiName: "OpportunityLineItem", fieldApiName: "Product_Condition__c" },
    { objectApiName: "OpportunityLineItem", fieldApiName: "Contract_Type_Software__c" },
    { objectApiName: "OpportunityLineItem", fieldApiName: "Subscription_Billing_Commitment__c" },
    { objectApiName: "OpportunityLineItem", fieldApiName: "Subscription_Billing_Plan__c" }
  ]);
  ...
  if (deal.lines_of_business === "Supplies") {
    issues.push({
      ...
      field: "linesOfBusiness",
      message: "You cannot choose Supplies. Please choose another line of business.",
      severity: "block"
    });
  }
  ...
  if (!checkPicklist(picklistCache, "Opportunity", "StageName", deal.stage_name)) {
    issues.push({
      code: "V-OPP-07",
      field: "stageName",
  ...
  if (!checkPicklist(picklistCache, "Opportunity", "CurrencyIsoCode", deal.currency_iso_code)) {
    issues.push({
      code: "V-OPP-08A",
      field: "currencyIsoCode",
  ...
  if (!checkPicklist(picklistCache, "Opportunity", "Lines_Of_Business__c", deal.lines_of_business)) {
    issues.push({
      code: "V-OPP-04A",
      field: "linesOfBusiness",
  ...
  if (!checkPicklist(picklistCache, "Opportunity", "Funding_Type__c", deal.funding_type)) {
    issues.push({
      code: "V-OPP-05A",
      field: "fundingType",
  ...
  if (deal.iwm_sales_order_req && deal.stage_name !== "Closed Won") {
    issues.push({
      ...
      message: "Sales Order Required can only be true when stage is Closed Won.",
      severity: "block"
    });
  }
  ...
  if (deal.iwm_sales_order_req && !deal.deliver_to_id) {
    issues.push({
      code: "V-OPP-11",
      field: "deliverToId",
      message: "Deliver To contact is recommended when Sales Order is requested",
      severity: "warn"
    });
  }
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!checkPicklist(picklistCache, "OpportunityLineItem", "Product_Condition__c", line.product_condition)) {
      issues.push({
        code: "V-OLI-05A",
        field: `${baseField}.productCondition`,
    ...
    const entryCurrency = await lookupPricebookCurrency(line.pricebook_entry_id);
    ...
    if (deal.funding_type === "Subscription" && !line.is_delivery_line) {
      ...
      if (!line.contract_type_software) {
        ...
        !checkPicklist(picklistCache, "OpportunityLineItem", "Contract_Type_Software__c", line.contract_type_software)
      ) {
        issues.push({
          code: "V-SUB-06A",
          field: `${baseField}.contractTypeSoftware`,
    ...
    if (deal.lines_of_business === "Software Licensing") {
      ...
      if (productCategory && productCategory !== "Software Licensing") {
        issues.push({
          ...
          field: `${baseField}.product2Id`,
          message: "Only software products can be added to Software Licensing opportunities.",
          severity: "block"
        });
      }
    }
